...
let lessThenAndEqual = number[0] <= number[3] //true
let greaterThenAndEqual = number[0] >= number[3] // false

// 동등(등급이나 정도가 같다) → boolean
let equal = number[0] == number[3] // false

// 다름(같지 않음) → boolean
let inequal = number[0] != number[3] // true

// 동일(똑같다) → boolean
let same = number[0] === number[3] // false
