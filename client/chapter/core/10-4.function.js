...
// - 재귀 단계(step) : 쟈귀 시, 반복적으로 처리할 일 (공통적인 처리)
// - 재귀 깊이(depth) : 재귀를 최대 볓 번 수행할 것인가?
...
// 성능적인 측면에서는 반복문이 더 좋음. 다만 재귀를 이요해면 코드 가독성이 높아짐 -> 유지보수 편함
...
const memoFibo = (num) => {
  if (num <= 0) return 0
  if (num <= 2) return 1
  if (memoFibo.cache[num]) return memoFibo[num]
  else;
  {
    return (memoFibo.cache[num] = memoFibo(num - 1) + memoFibo(num - 2))
  }
}
